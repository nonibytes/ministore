Worth fixing (these will unblock meaningful tests)

1. **`PutJSON` is effectively broken on SQLite right now**
   `ops/put.go` calls `ExecContext(... RETURNING ...)` and then `LastInsertId()`. With `RETURNING`, you should `QueryRowContext` + `Scan`. As-is, inserts/updates won’t reliably work, and any integration tests around `PutJSON/Search/Delete` will fail.

2. **CLI `put -p ... --set key=value` can’t work**
   `flag.FlagSet` will error on unknown `--set` because it isn’t defined, so you never reach your manual parsing of `fs.Args()`.

3. **Cursor safety**
   There’s no query-hash binding in `ops` cursor payloads, so users can pass a cursor from a different query and get undefined pagination. (Not required for correctness, but worth addressing.)

---

## Patch 1: fix SQLite upsert to actually return id/created_at

```go
// File: ministore/ops/put.go
// Replace the existing upsertItem(...) with this version.
...
## Patch 2: make CLI `--set` actually work (repeatable flag)

```go
// File: cmd/ministore/main.go
...
```go
// File: cmd/ministore/main.go
// In handlePut(...), replace the manual --set parsing with a real flag:
...
func handlePut(ctx context.Context, args []string) {
	...
	// ... unchanged open index, jsonMode handling ...
...
# New tests (SQLite integration)

These exercise `Create/Open`, `PutJSON`, `Search`, `Delete`, `DiscoverValues`, `Stats`, plus cursor pagination in **recency** mode (no FTS dependency). They will fail without the `upsertItem` fix above.

```go
// File: ministore/integration_sqlite_test.go
...
# Tighten two existing normalization tests (so they assert behavior)

```go
// File: ministore/query/normalize_test.go
// Replace the two “non-asserting” tests with these.
...
These tests match your current semantics while making failures obvious.

---

If you apply the two small fixes + add the new test file, you’ll get coverage over the critical runtime paths that were previously untested (`PutJSON` especially).
