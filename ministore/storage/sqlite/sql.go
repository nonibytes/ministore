...
func (u upsertItem) BuildInsert(path string, dataJSON []byte, createdAtMS, updatedAtMS int64) (string, []any) {
	sql := `INSERT INTO items(path, data_json, created_at, updated_at)
		VALUES(?1, ?2, ?3, ?4)
		ON CONFLICT(path) DO NOTHING
		RETURNING id, created_at`
	return sql, []any{path, string(dataJSON), createdAtMS, updatedAtMS}
}
...
var SQLTemplates = storage.SQL{
	...
	ScanPaths:               "SELECT path FROM items WHERE substr(path, 1, length(?1)) = ?1 ORDER BY path COLLATE BINARY",
	...
	AdjustDocFreq:           "UPDATE kw_dict SET doc_freq = MAX(doc_freq + ?2, 0) WHERE id = ?1",
